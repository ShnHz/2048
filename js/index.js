...
function drawBackground() {
  ...
  for (var i = 0; i < game.length; i++) {
    ...
    tileDiv.style.top = y * 87.5 + 'px';
    tileDiv.style.left = x * 87.5 + 'px';
...
function positionTile(tile, elm) {
  ...
  elm.style.top = y * 87.5 + 'px';
  elm.style.left = x * 87.5 + 'px';
}
    ...
    if (tile) {
      if (isNew) {
        (function () {
          ...
          tileDiv.classList.add('tile', 'tile--' + tilecss);
          tileDiv.id = tile.id;
          setTimeout(function () {
...
var buttons = document.querySelectorAll(".restart");
...
function recordingMoudule() {
  lastContainer = $("#tile-container").html();
}
...
function laststep() {
  $("#tile-container").html("");
  $("#tile-container").html(lastContainer);
  $("#score").html(lastscore);
  game = lastlayout;
  score = lastscore;

  $("#laststep").removeAttr("onclick");
  $("#laststep").html("只能退一步哦");
  $("#laststep").css("background", "#d0cccb");
  lastbtn = true;
}
